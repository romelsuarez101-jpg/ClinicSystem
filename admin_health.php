...
$moderateRisk  = $conn->query("SELECT COUNT(*) AS c FROM health_records WHERE cvd_category='Moderate Risk'")->fetch_assoc()['c'];
$highRisk      = $conn->query("SELECT COUNT(*) AS c FROM health_records WHERE cvd_category='High Risk'")->fetch_assoc()['c'];
$clinicCount   = $conn->query("SELECT COUNT(*) AS c FROM health_records WHERE source='clinic'")->fetch_assoc()['c'];
...
<div class="page-header">
  ...
  <a href="admin_health_check.php" class="btn btn-primary">
    🩺 New Clinic Assessment
  </a>
</div>
...
<div class="stats-grid" style="margin-bottom:24px;">
  ...
  <div class="stat-card yellow">
    ...
    <div class="stat-value"><?= $moderateRisk ?></div>
    <div class="stat-sub">students</div>
...
<div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;align-items:center;">
  ...
  <div style="display:flex;gap:6px;">
    ...
    <a href="?risk=low&source=<?= $filterSource ?>"
       class="btn <?= $filterRisk==='low'?'btn-primary':'btn-secondary' ?> btn-sm"
       style="<?= $filterRisk==='low'?'':'color:var(--green);border-color:rgba(63,185,80,.3)' ?>">
      ✅ Low
    </a>
    <a href="?risk=moderate&source=<?= $filterSource ?>"
       class="btn <?= $filterRisk==='moderate'?'btn-primary':'btn-secondary' ?> btn-sm"
       style="<?= $filterRisk==='moderate'?'':'color:var(--yellow);border-color:rgba(227,179,65,.3)' ?>">
      ⚠️ Moderate
    </a>
    <a href="?risk=high&source=<?= $filterSource ?>"
       class="btn <?= $filterRisk==='high'?'btn-primary':'btn-secondary' ?> btn-sm"
       style="<?= $filterRisk==='high'?'':'color:var(--red);border-color:rgba(248,81,73,.3)' ?>">
      🚨 High
    </a>
  </div>
  ...
  <div style="display:flex;gap:6px;">
    <a href="?source=all&risk=<?= $filterRisk ?>"
       class="btn <?= $filterSource==='all'?'btn-primary':'btn-secondary' ?> btn-sm">
      All Sources
    </a>
    <a href="?source=self&risk=<?= $filterRisk ?>"
       class="btn <?= $filterSource==='self'?'btn-primary':'btn-secondary' ?> btn-sm">
      👤 Self
    </a>
    <a href="?source=clinic&risk=<?= $filterRisk ?>"
       class="btn <?= $filterSource==='clinic'?'btn-primary':'btn-secondary' ?> btn-sm">
      🏥 Clinic
    </a>
  </div>
