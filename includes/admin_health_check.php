...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id'])) {
    ...
    $bmi_f      = (float)$bmi;
    $cvd_risk_f = (float)$cvd_risk;
    $hs_int     = (int)$health_score;

    $save->bind_param("iisdddsiiisddiiidsis s",
        $user_id, $age, $sex,
        $height, $weight, $bmi_f, $bmi_cat,
        $systolic, $diastolic, $bp_cat,
        $total_chol, $hdl_chol,
        $smoking, $diabetes, $bp_treated,
        $cvd_risk_f, $cvd_cat,
        $hs_int, $health_cat,
        $source, $notes
    );
...
$allStudents = $conn->query("SELECT user_id, full_name, student_id, grade FROM users WHERE status='Active' ORDER BY full_name ASC");
...
<div class="alert alert-success" style="margin-bottom:24px;">
  🏥 <strong>Clinic Assessment:</strong> Results entered by the nurse are marked as <strong>Clinic-Verified</strong> and are considered more accurate than self-reported data.
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;">
  ...
  <div class="form-card">
    ...
    <form method="GET" action="admin_health_check.php" style="margin-bottom:20px;">
    ...
    <div style="margin-bottom:20px;">
      <div style="font-size:12px;color:var(--text3);margin-bottom:8px;text-align:center;">— or select from list —</div>
    ...
    <form method="POST" action="admin_health_check.php">
      ...
      <div class="form-grid">
        ...
        <div class="field span2">
          <label>Risk Factors (as reported by patient)</label>
  ...
  <?php if ($result && $patient): ?>
    ...
    <!-- Recommendations -->
    <div class="form-card">
      <h3 style="font-family:'Playfair Display',serif;font-size:15px;margin-bottom:14px;">💡 Clinical Recommendations</h3>
      ...
      <div style="margin-top:12px;padding:10px;background:rgba(63,185,80,.06);border:1px solid rgba(63,185,80,.2);border-radius:8px;">
        <div style="font-size:11px;color:var(--green);">
          ✅ <strong>Clinic-verified</strong> — Recorded by <?= htmlspecialchars(getCurrentUser()) ?> on <?= date('F d, Y h:i A') ?>
        </div>
      </div>
      <div style="margin-top:8px;">
        <a href="admin_health.php" class="btn btn-secondary btn-sm">← View All Records</a>
        <a href="admin_health_check.php" class="btn btn-primary btn-sm" style="margin-left:8px;">🩺 New Assessment</a>
      </div>
    </div>
  ...
  <?php else: ?>
    <div class="form-card" style="text-align:center;padding:48px 32px;">
      ...
      <div style="margin-top:24px;padding:14px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;text-align:left;">
        <div style="font-size:12px;color:var(--text3);margin-bottom:8px;font-weight:600;">WHY CLINIC ASSESSMENT?</div>
